...
int __cdecl MyMediaProcThreadUserInit( MediaProcThread * MediaProcThreadPt )
{
	...
	if( g_MediaInfoPt->m_UseWhatXfrPrtcl == 0 ) //如果使用TCP协议。
	{
		...
		{
			if( g_MediaInfoPt->m_TcpSrvrSokt.Init( 4, g_MediaInfoPt->m_IPAddrVarStrPt->m_StrPt, g_MediaInfoPt->m_PortVarStrPt->m_StrPt, 1, 1, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) //如果创建并初始化已监听的本端TCP协议服务端套接字成功。
			{
				char p_LclNodeAddrPt[INET6_ADDRSTRLEN];
				char p_LclNodePortPt[6];

				if( g_MediaInfoPt->m_TcpSrvrSokt.GetLclAddr( NULL, p_LclNodeAddrPt, p_LclNodePortPt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果获取已监听的本端TCP协议服务端套接字绑定的本地节点地址和端口失败。
				{
			...
			{
				if( g_MediaInfoPt->m_TcpSrvrSokt.Accept( NULL, p_RmtNodeAddrPt, p_RmtNodePortPt, 1, &g_MediaInfoPt->m_TcpClntSokt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
				{
					if( g_MediaInfoPt->m_TcpClntSokt.m_TcpClntSoktPt != NULL ) //如果用已监听的本端TCP协议服务端套接字接受远端TCP协议客户端套接字的连接成功。
					{
						g_MediaInfoPt->m_TcpSrvrSokt.Destroy( NULL ); //关闭并销毁已创建的本端TCP协议服务端套接字，防止还有其他远端TCP协议客户端套接字继续连接。
		...
		else if( g_MediaInfoPt->m_IsCreateSrvrOrClnt == 0 ) //如果是创建本端TCP协议客户端套接字连接远端TCP协议服务端套接字。
		{
			...
			{
				if( g_MediaInfoPt->m_TcpClntSokt.Init( 4, g_MediaInfoPt->m_IPAddrVarStrPt->m_StrPt, g_MediaInfoPt->m_PortVarStrPt->m_StrPt, NULL, NULL, 5000, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) //如果创建并初始化本端TCP协议客户端套接字，并连接已监听的远端TCP协议服务端套接字成功。
				{
					char p_LclNodeAddrPt[INET6_ADDRSTRLEN];
					char p_LclNodePortPt[6];
					char p_RmtNodeAddrPt[INET6_ADDRSTRLEN];
					...
					if( g_MediaInfoPt->m_TcpClntSokt.GetLclAddr( NULL, p_LclNodeAddrPt, p_LclNodePortPt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 )
					{
					...
					if( g_MediaInfoPt->m_TcpClntSokt.GetRmtAddr( NULL, p_RmtNodeAddrPt, p_RmtNodePortPt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 )
					{
		...
		if( g_MediaInfoPt->m_TcpClntSokt.SetNoDelay( 1, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果设置已连接的本端TCP协议客户端套接字的Nagle延迟算法状态为禁用失败。
		{
		...
		if( g_MediaInfoPt->m_TcpClntSokt.SetSendBufSz( 128 * 1024, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果设置已连接的本端TCP协议客户端套接字的发送缓冲区内存大小失败。
        {
		...
		if( g_MediaInfoPt->m_TcpClntSokt.SetRecvBufSz( 128 * 1024, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果设置已连接的本端TCP协议客户端套接字的接收缓冲区内存大小失败。
        {
	...
	else //如果使用UDP协议。
    {
        ...
        {
            if( g_MediaInfoPt->m_UdpSokt.Init( 4, g_MediaInfoPt->m_IPAddrVarStrPt->m_StrPt, g_MediaInfoPt->m_PortVarStrPt->m_StrPt, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) //如果创建并初始化已监听的本端UDP协议套接字成功。
            {
                char p_LclNodeAddrPt[INET6_ADDRSTRLEN];
				char p_LclNodePortPt[6];

                if( g_MediaInfoPt->m_UdpSokt.GetLclAddr( NULL, p_LclNodeAddrPt, p_LclNodePortPt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果获取已监听的本端UDP协议套接字绑定的本地节点地址和端口失败。
                {
            ...
            {
                if( g_MediaInfoPt->m_UdpSokt.RecvPkt( NULL, p_RmtNodeAddrPt, p_RmtNodePortPt, g_MediaInfoPt->m_TmpBytePt, g_MediaInfoPt->m_TmpByteSz, &p_TmpSz, 1, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
                {
                    if( p_TmpSz != -1 ) //如果用已监听的本端UDP协议套接字开始接收远端UDP协议套接字发送的一个数据包成功。
                    {
                        if( ( p_TmpSz == 1 ) && ( g_MediaInfoPt->m_TmpBytePt[0] == PKT_TYP_CNCT_HTBT ) ) //如果是连接请求包。
                        {
                            ...
                            while( true ) //循环发送连接请求包，并接收连接应答包。
                            {
                                ...
                                if( g_MediaInfoPt->m_UdpSokt.SendPkt( 4, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, 1, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 )
                                {
                                ...
                                {
                                    if( g_MediaInfoPt->m_UdpSokt.RecvPkt( NULL, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, g_MediaInfoPt->m_TmpByteSz, &p_TmpSz, 1000, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
                                    {
                                        if( p_TmpSz != -1 ) //如果用已监听的本端UDP协议套接字开始接收远端UDP协议套接字发送的一个数据包成功。
                                        {
        ...
        else if( g_MediaInfoPt->m_IsCreateSrvrOrClnt == 0 ) //如果是创建本端UDP协议套接字连接远端UDP协议套接字。
        {
            ...
            if( g_MediaInfoPt->m_UdpSokt.Init( 4, NULL, NULL, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) //如果创建并初始化已监听的本端UDP协议套接字成功。
            {
                char p_LclNodeAddrPt[INET6_ADDRSTRLEN];
				char p_LclNodePortPt[6];
            ...
            if( g_MediaInfoPt->m_UdpSokt.Connect( 4, g_MediaInfoPt->m_IPAddrVarStrPt->m_StrPt, g_MediaInfoPt->m_PortVarStrPt->m_StrPt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 )
            {
            ...
            if( g_MediaInfoPt->m_UdpSokt.GetRmtAddr( NULL, p_RmtNodeAddrPt, p_RmtNodePortPt, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 )
            {
		...
		if( g_MediaInfoPt->m_UdpSokt.SetSendBufSz( 128 * 1024, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果设置已监听的本端UDP协议套接字的发送缓冲区内存大小失败。
        {
		...
		if( g_MediaInfoPt->m_UdpSokt.SetRecvBufSz( 128 * 1024, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) != 0 ) //如果设置已监听的本端UDP协议套接字的接收缓冲区内存大小失败。
        {
	...
	switch( g_MediaInfoPt->m_UseWhatRecvOutputFrame ) //使用什么接收输出帧。
	{
	    ...
	    {
			//创建并初始化接收音频输出帧链表。
			if( g_MediaInfoPt->m_RecvAudioOutputFrameLnkLst.Init( LNKLST_BUF_AUTO_ADJ_METH_FREENUMBER, MediaProcThreadPt->m_AudioOutput.m_FrameLen * 2, MediaProcThreadPt->m_AudioOutput.m_FrameLen * 2, SIZE_MAX, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
			{
			...
			//创建并初始化接收视频输出帧链表。
			if( g_MediaInfoPt->m_RecvVideoOutputFrameLnkLst.Init( LNKLST_BUF_AUTO_ADJ_METH_FREENUMBER, MediaProcThreadPt->m_AudioOutput.m_FrameLen * 2, MediaProcThreadPt->m_AudioOutput.m_FrameLen * 2, SIZE_MAX, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
			{
	    ...
	    {
			//初始化音频自适应抖动缓冲器。
            if( AAjbInit( &g_MediaInfoPt->m_AAjbPt, MediaProcThreadPt->m_AudioOutput.m_SamplingRate, MediaProcThreadPt->m_AudioOutput.m_FrameLen, 1, 1, 0, g_MediaInfoPt->m_AAjbMinNeedBufFrameCnt, g_MediaInfoPt->m_AAjbMaxNeedBufFrameCnt, g_MediaInfoPt->m_AAjbAdaptSensitivity, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
            {
            ...
            //初始化视频自适应抖动缓冲器。
            if( VAjbInit( &g_MediaInfoPt->m_VAjbPt, 1, g_MediaInfoPt->m_VAjbMinNeedBufFrameCnt, g_MediaInfoPt->m_VAjbMaxNeedBufFrameCnt, g_MediaInfoPt->m_VAjbAdaptSensitivity, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
            {
...
int __cdecl MyMediaProcThreadUserProcess( MediaProcThread * MediaProcThreadPt )
{
    ...
    //接收远端发送过来的一个数据包。
	if( ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 0 ) && ( g_MediaInfoPt->m_TcpClntSokt.RecvPkt( g_MediaInfoPt->m_TmpBytePt, g_MediaInfoPt->m_TmpByteSz, &p_TmpSz, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) ||
        ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 1 ) && ( g_MediaInfoPt->m_UdpSokt.RecvPkt( NULL, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, g_MediaInfoPt->m_TmpByteSz, &p_TmpSz, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) )
    {
		if( p_TmpSz != -1 ) //如果用已连接的本端套接字开始接收连接的远端套接字发送的一个数据包成功。
		{
			...
			else if( g_MediaInfoPt->m_TmpBytePt[0] == PKT_TYP_AFRAME ) //如果是音频输出帧包。
			{
				...
				if( MediaProcThreadPt->m_AudioOutput.m_IsUseAudioOutput != 0 ) //如果要使用音频输出。
				{
					...
					switch( g_MediaInfoPt->m_UseWhatRecvOutputFrame ) //使用什么接收输出帧。
					{
						case 0: //如果使用链表。
						{
							...
							{
								if( g_MediaInfoPt->m_RecvAudioOutputFrameLnkLst.PutTail( g_MediaInfoPt->m_TmpBytePt + 1 + 4, p_TmpSz - 1 - 4, 1, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
								{
                ...
                if( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 1 ) && ( p_TmpSz == 1 + 4 ) ) //如果是使用UDP协议，且本音频输出帧为无语音活动。
                {
                    ...
                    if( g_MediaInfoPt->m_UdpSokt.SendPkt( 4, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, 1 + 4, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
                    {
                ...
                if( MediaProcThreadPt->m_VideoOutput.m_IsUseVideoOutput ) //如果要使用视频输出。
                {
                    ...
                    switch( g_MediaInfoPt->m_UseWhatRecvOutputFrame ) //使用什么接收输出帧。
                    {
                        case 0: //如果使用链表。
                        {
							...
							{
								if( g_MediaInfoPt->m_RecvVideoOutputFrameLnkLst.PutTail( g_MediaInfoPt->m_TmpBytePt + 1 + 4, p_TmpSz - 1 - 4, 1, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 )
								{
    ...
    if( p_TmpUint64 - g_MediaInfoPt->m_LastPktSendTime >= 100 ) //如果超过100毫秒没有发送任何数据包，就发送一个心跳包。
    {
        ...
        if( ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 0 ) && ( g_MediaInfoPt->m_TcpClntSokt.SendPkt( g_MediaInfoPt->m_TmpBytePt, 1, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) ||
            ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 1 ) && ( g_MediaInfoPt->m_UdpSokt.SendPkt( 4, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, 1, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) )
        {
...
void __cdecl MyMediaProcThreadUserDestroy( MediaProcThread * MediaProcThreadPt )
{
	...
	if( ( MediaProcThreadPt->m_ExitFlag == 1 ) && ( ( g_MediaInfoPt->m_TcpClntSokt.m_TcpClntSoktPt != NULL ) || ( ( g_MediaInfoPt->m_UdpSokt.m_UdpSoktPt != NULL ) && ( g_MediaInfoPt->m_UdpSokt.GetRmtAddr( NULL, NULL, NULL, 0, NULL ) == 0 ) ) ) ) //如果本线程接收到退出请求，且本端TCP协议客户端套接字类对象不为空或本端UDP协议套接字类对象不为空且已连接远端。
    {
        //循环发送退出包。
        ...
        if( g_MediaInfoPt->m_IsRecvExitPkt == 0 ) //如果没有接收到退出包。
        {
            while( true ) //循环接收退出包。
            {
                if( ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 0 ) && ( g_MediaInfoPt->m_TcpClntSokt.RecvPkt( g_MediaInfoPt->m_TmpBytePt, g_MediaInfoPt->m_TmpByteSz, &p_TmpSz, 5000, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) ||
                    ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 1 ) && ( g_MediaInfoPt->m_UdpSokt.RecvPkt( NULL, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, g_MediaInfoPt->m_TmpByteSz, &p_TmpSz, 5000, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) )
                {
                    if( p_TmpSz != -1 ) //如果用已连接的本端套接字开始接收连接的远端套接字发送的一个数据包成功。
                    {
						FuncGetTimeAsMsec( &g_MediaInfoPt->m_LastPktRecvTime ); //记录最后一个数据包的接收时间。
    ...
    {
        g_MediaInfoPt->m_TcpSrvrSokt.Destroy( NULL ); //关闭并销毁已创建的本端TCP协议服务端套接字。
    ...
    {
        g_MediaInfoPt->m_TcpClntSokt.Destroy( ( uint16_t ) -1, NULL ); //关闭并销毁已创建的本端TCP协议客户端套接字。
    ...
    {
        g_MediaInfoPt->m_UdpSokt.Destroy( NULL ); //关闭并销毁已创建的本端UDP协议套接字。
    ...
    {
		g_MediaInfoPt->m_RecvAudioOutputFrameLnkLst.Destroy( NULL );
    ...
    {
		g_MediaInfoPt->m_RecvVideoOutputFrameLnkLst.Destroy( NULL );
...
void __cdecl MyMediaProcThreadUserReadAudioVideoInputFrame( MediaProcThread * MediaProcThreadPt, int16_t * PcmAudioInputFramePt, int16_t * PcmAudioResultFramePt, int32_t VoiceActSts, uint8_t * EncoderAudioInputFramePt, size_t EncoderAudioInputFrameLen, int32_t EncoderAudioInputFrameIsNeedTrans, uint8_t * YU12VideoInputFramePt, int32_t YU12VideoInputFrameWidth, int32_t YU12VideoInputFrameHeight, uint8_t * EncoderVideoInputFramePt, size_t EncoderVideoInputFrameLen )
{
	...
	if( PcmAudioInputFramePt != NULL ) //如果要使用音频输入。
    {
        ...
        if( p_FramePktLen != 1 + 4 ) //如果本音频输入帧为有语音活动，就发送。
        {
            ...
            if( ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 0 ) && ( g_MediaInfoPt->m_TcpClntSokt.SendPkt( g_MediaInfoPt->m_TmpBytePt, p_FramePktLen, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) ||
                ( ( g_MediaInfoPt->m_UseWhatXfrPrtcl == 1 ) && ( g_MediaInfoPt->m_UdpSokt.SendPkt( 4, NULL, NULL, g_MediaInfoPt->m_TmpBytePt, p_FramePktLen, 0, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ) == 0 ) ) )
            {
				FuncGetTimeAsMsec( &g_MediaInfoPt->m_LastPktSendTime ); //设置最后一个数据包的发送时间。
		...
		switch( g_MediaInfoPt->m_UseWhatRecvOutputFrame ) //使用什么接收音频输出帧。
		{
			case 0: //如果使用链表。
			{
				...
				if( m_TmpSz != 0 )
				{
					...
					g_MediaInfoPt->m_RecvAudioOutputFrameLnkLst.GetHead( NULL, g_MediaInfoPt->m_TmpByte2Pt, g_MediaInfoPt->m_TmpByte2Sz, NULL, &m_TmpSz, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ); //获取接收音频输出帧链表的第一个音频输出帧。
					g_MediaInfoPt->m_RecvAudioOutputFrameLnkLst.DelHead( 0, MediaProcThreadPt->m_ErrInfoVarStrPt ); //删除接收音频输出帧链表的第一个音频输出帧。
    ...
    switch( g_MediaInfoPt->m_UseWhatRecvOutputFrame ) //使用什么接收输出帧。
    {
        case 0: //如果使用链表。
        {
            ...
            if( m_TmpSz != 0 ) //如果接收视频输出帧链表不为空。
            {
				...
				g_MediaInfoPt->m_RecvVideoOutputFrameLnkLst.GetHead( NULL, g_MediaInfoPt->m_TmpByte3Pt, g_MediaInfoPt->m_TmpByte3Sz, NULL, &m_TmpSz, 0, MediaProcThreadPt->m_ErrInfoVarStrPt ); //获取接收视频输出帧链表的第一个视频输出帧。
				g_MediaInfoPt->m_RecvVideoOutputFrameLnkLst.DelHead( 0, MediaProcThreadPt->m_ErrInfoVarStrPt ); //删除接收视频输出帧链表的第一个视频输出帧。
		...
		case WM_COMMAND:
		{
			switch( LOWORD( wParam ) )
			{
						...
						MediaProcThreadSetIsUseAudioInput( g_MediaProcThreadPt,
														   ...
														   ( IsDlgButtonChecked( g_SettingWndHdl, IDC_UseAudioSamplingRate8000 ) == BST_CHECKED ) ? 8000 :
														     ( IsDlgButtonChecked( g_SettingWndHdl, IDC_UseAudioSamplingRate16000 ) == BST_CHECKED ) ? 16000 :
														       ( IsDlgButtonChecked( g_SettingWndHdl, IDC_UseAudioSamplingRate32000 ) == BST_CHECKED ) ? 32000 : 
														         ( IsDlgButtonChecked( g_SettingWndHdl, IDC_UseAudioSamplingRate48000 ) == BST_CHECKED ) ? 48000 : 0,
				...
				case IDC_UseEffectLow: //效果等级：低。
				{
					...
					CheckRadioButton( g_SettingWndHdl, IDC_UseAudioSamplingRate8000, IDC_UseAudioSamplingRate48000, IDC_UseAudioSamplingRate8000 );
				...
				case IDC_UseEffectMid: //效果等级：中。
				{
					...
					CheckRadioButton( g_SettingWndHdl, IDC_UseAudioSamplingRate8000, IDC_UseAudioSamplingRate48000, IDC_UseAudioSamplingRate16000 );
					CheckRadioButton( g_SettingWndHdl, IDC_UseAudioFrameLen10ms, IDC_UseAudioFrameLen30ms, IDC_UseAudioFrameLen20ms );
				...
				case IDC_UseEffectPremium: //效果等级：特。
				{
					...
					CheckRadioButton( g_SettingWndHdl, IDC_UseAudioSamplingRate8000, IDC_UseAudioSamplingRate48000, IDC_UseAudioSamplingRate32000 );
					CheckRadioButton( g_SettingWndHdl, IDC_UseAudioFrameLen10ms, IDC_UseAudioFrameLen30ms, IDC_UseAudioFrameLen20ms );
					CheckRadioButton( g_SettingWndHdl, IDC_UseNoAec, IDC_UseSpeexWebRtcAec, IDC_UseSpeexWebRtcAec );
					CheckRadioButton( g_SettingWndHdl, IDC_UseNoNs, IDC_UseRNNoise, IDC_UseRNNoise );
					CheckDlgButton( g_SettingWndHdl, IDC_IsUseSpeexPprocOther, BST_CHECKED );
				...
				case IDC_UseBitrateLow: //比特率等级：低。
				{
					...
					SetWindowText( GetDlgItem( g_OpenH264CodecSettingWndHdl, IDC_OpenH264EncoderEncodedBitrate ), "10" );
