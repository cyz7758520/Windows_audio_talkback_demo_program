...
typedef struct MediaPocsThrd
{
	...
	Vstr * m_AdoVdoInptOtptAviFileFullPathVstrPt; //存放音视频输入输出Avi文件完整路径动态字符串的指针。
	size_t m_AdoVdoInptOtptAviFileWrBufSzByt; //存放音视频输入输出Avi文件写入缓冲区的大小，单位为字节。
	int32_t m_AdoVdoInptOtptAviFileIsSaveAdoInpt; //存放音视频输入输出Avi文件是否保存音频输入，为非0表示要保存，为0表示不保存。
	int32_t m_AdoVdoInptOtptAviFileIsSaveAdoOtpt; //存放音视频输入输出Avi文件是否保存音频输出，为非0表示要保存，为0表示不保存。
	int32_t m_AdoVdoInptOtptAviFileIsSaveVdoInpt; //存放音视频输入输出Avi文件是否保存视频输入，为非0表示要保存，为0表示不保存。
	int32_t m_AdoVdoInptOtptAviFileIsSaveVdoOtpt; //存放音视频输入输出Avi文件是否保存视频输出，为非0表示要保存，为0表示不保存。
	uint32_t m_AdoVdoInptOtptAviFilePcmAdoInptSrcStrmIdx; //存放音视频输入输出Avi文件Pcm格式音频输入原始流的索引。
	uint32_t m_AdoVdoInptOtptAviFilePcmAdoInptRsltStrmIdx; //存放音视频输入输出Avi文件Pcm格式音频输入结果流的索引。
	int32_t m_AdoVdoInptOtptAviFileAdoInptStrmTimeStampIsReset; //存放音视频输入输出Avi文件音频输入流时间戳是否重置，为非0表示要重置，为0表示不重置。
	uint32_t m_AdoVdoInptOtptAviFilePcmAdoOtptSrcStrmIdx; //存放音视频输入输出Avi文件Pcm格式音频输出原始流的索引。
	int32_t m_AdoVdoInptOtptAviFileAdoOtptStrmTimeStampIsReset; //存放音视频输入输出Avi文件音频输出流时间戳是否重置，为非0表示要重置，为0表示不重置。
	uint64_t m_AdoVdoInptOtptAviFileAdoInptOtptStrmCurTimeStampMsec; //存放音视频输入输出Avi文件音频输入输出流的当前时间戳，单位为毫秒。
	uint32_t m_AdoVdoInptOtptAviFileEncdVdoInptRsltStrmIdx; //存放音视频输入输出Avi文件已编码格式视频输入结果流的索引。
	std::map< uint32_t, uint32_t > * m_AdoVdoInptOtptAviFileEncdVdoOtptSrcStrmIdxMapPt; //存放音视频输入输出Avi文件已编码格式视频输出原始流的索引映射的指针。
...
public:
	...
	//用户定义的读取音视频输入帧函数。
	virtual void UserReadAdoVdoInptFrm( int16_t * PcmAdoInptSrcFrmPt, int16_t * PcmAdoInptRsltFrmPt, int32_t PcmAdoInptFrmLenUnit, int32_t PcmAdoInptRsltFrmVoiceActSts,
										uint8_t * EncdAdoInptRsltFrmPt, size_t EncdAdoInptRsltFrmLenByt, int32_t EncdAdoInptRsltFrmIsNeedTrans,
										uint8_t * BgraVdoInptSrcFrmPt, int32_t BgraVdoInptSrcFrmWidth, int32_t BgraVdoInptSrcFrmHeight, size_t BgraVdoInptSrcFrmLenByt,
										uint8_t * YU12VdoInptRsltFrmPt, int32_t YU12VdoInptRsltFrmWidth, int32_t YU12VdoInptRsltFrmHeight, size_t YU12VdoInptRsltFrmLenByt,
	...
	int SetAdoInptIsSaveAdoToWaveFile( int32_t IsSaveAdoToWaveFile, const Vstr * AdoInptSrcWaveFileFullPathVstrPt, const Vstr * AdoInptRsltWaveFileFullPathVstrPt, size_t AdoInptWaveFileWrBufSzByt, VstrCls * ErrInfoVstrPt ) { return MediaPocsThrdSetAdoInptIsSaveAdoToWaveFile( m_MediaPocsThrdPt, IsSaveAdoToWaveFile, AdoInptSrcWaveFileFullPathVstrPt, AdoInptRsltWaveFileFullPathVstrPt, AdoInptWaveFileWrBufSzByt, ( ErrInfoVstrPt != NULL ) ? ErrInfoVstrPt->m_VstrPt : NULL ); }
	...
	int SetAdoOtptIsSaveAdoToWaveFile( int32_t IsSaveAdoToWaveFile, const Vstr * AdoOtptSrcWaveFileFullPathVstrPt, size_t AdoOtptWaveFileWrBufSzByt, VstrCls * ErrInfoVstrPt ) { return MediaPocsThrdSetAdoOtptIsSaveAdoToWaveFile( m_MediaPocsThrdPt, IsSaveAdoToWaveFile, AdoOtptSrcWaveFileFullPathVstrPt, AdoOtptWaveFileWrBufSzByt, ( ErrInfoVstrPt != NULL ) ? ErrInfoVstrPt->m_VstrPt : NULL ); }
