...
typedef struct //音频输入。
{
	...
	size_t m_FrmLenMsec; //存放帧长度，单位为毫秒，取值只能为10毫秒的倍数。
	size_t m_FrmLenUnit; //存放帧长度，单位为采样单元，取值只能为10毫秒的倍数。例如：8000Hz的10毫秒为80、20毫秒为160、30毫秒为240，16000Hz的10毫秒为160、20毫秒为320、30毫秒为480，32000Hz的10毫秒为320、20毫秒为640、30毫秒为960，48000Hz的10毫秒为480、20毫秒为960、30毫秒为1440。
	size_t m_FrmLenData; //存放帧长度，单位为采样数据，取值只能为10毫秒的倍数。例如：8000Hz的10毫秒为80、20毫秒为160、30毫秒为240，16000Hz的10毫秒为160、20毫秒为320、30毫秒为480，32000Hz的10毫秒为320、20毫秒为640、30毫秒为960，48000Hz的10毫秒为480、20毫秒为960、30毫秒为1440。
	size_t m_FrmLenByt; //存放帧长度，单位为字节，取值只能为10毫秒的倍数。例如：8000Hz的10毫秒为80*2、20毫秒为160*2、30毫秒为240*2，16000Hz的10毫秒为160*2、20毫秒为320*2、30毫秒为480*2，32000Hz的10毫秒为320*2、20毫秒为640*2、30毫秒为960*2，48000Hz的10毫秒为480*2、20毫秒为960*2、30毫秒为1440*2。
	
	int32_t m_IsUseSystemAecNsAgc; //存放是否使用系统自带声学回音消除器、噪音抑制器和自动增益控制器，为0表示不使用，为非0表示要使用。
	...
	{
		SpeexAec * m_Pt; //存放指针。
		int32_t m_FilterLenMsec; //存放滤波器长度，单位为毫秒。
		int32_t m_IsUseRec; //存放是否使用残余回音消除，为非0表示要使用，为0表示不使用。
		float m_EchoMutp; //存放在残余回音消除时，残余回音倍数，倍数越大消除越强，取值区间为[0.0,100.0]。
		float m_EchoCntu; //存放在残余回音消除时，残余回音持续系数，系数越大消除越强，取值区间为[0.0,0.9]。
		int32_t m_EchoSupes; //存放在残余回音消除时，残余回音最大衰减分贝值，分贝值越小衰减越大，取值区间为[-2147483648,0]。
		int32_t m_EchoSupesAct; //存放在残余回音消除时，有近端语音活动时残余回音最大衰减分贝值，分贝值越小衰减越大，取值区间为[-2147483648,0]。
		int32_t m_IsSaveMemFile; //存放是否保存内存块到文件，为非0表示要保存，为0表示不保存。
		Vstr * m_MemFileFullPathVstrPt; //存放内存块文件完整路径动态字符串指针。
	} m_SpeexAec;
	...
	struct //存放SpeexWebRtc三重声学回音消除器。
	{
		...
		int32_t m_SpeexAecFilterLenMsec; //存放Speex声学回音消除器滤波器长度，单位为毫秒。
		int32_t m_SpeexAecIsUseRec; //存放Speex声学回音消除器是否使用残余回音消除，为非0表示要使用，为0表示不使用。
		float m_SpeexAecEchoMutp; //存放Speex声学回音消除器在残余回音消除时，残余回音倍数，倍数越大消除越强，取值区间为[0.0,100.0]。
		float m_SpeexAecEchoCntu; //存放Speex声学回音消除器在残余回音消除时，残余回音持续系数，系数越大消除越强，取值区间为[0.0,0.9]。
		int32_t m_SpeexAecEchoSupes; //存放Speex声学回音消除器在残余回音消除时，残余回音最大衰减分贝值，分贝值越小衰减越大，取值区间为[-2147483648,0]。
		int32_t m_SpeexAecEchoSupesAct; //存放Speex声学回音消除器在残余回音消除时，有近端语音活动时残余回音最大衰减分贝值，分贝值越小衰减越大，取值区间为[-2147483648,0]。
		int32_t m_WebRtcAecmIsUseCNGMode; //存放WebRtc定点版声学回音消除器是否使用舒适噪音生成模式，为非0表示要使用，为0表示不使用。
		int32_t m_WebRtcAecmEchoMode; //存放WebRtc定点版声学回音消除器消除模式，消除模式越高消除越强，取值区间为[0,4]。
		int32_t m_WebRtcAecmDelay; //存放WebRtc定点版声学回音消除器回音延迟，单位为毫秒，取值区间为[-2147483648,2147483647]，为0表示自适应设置。
		int32_t m_WebRtcAecEchoMode; //存放WebRtc浮点版声学回音消除器消除模式，消除模式越高消除越强，取值区间为[0,2]。
		int32_t m_WebRtcAecDelay; //存放WebRtc浮点版声学回音消除器回音延迟，单位为毫秒，取值区间为[-2147483648,2147483647]，为0表示自适应设置。
	...
	struct //存放Speex预处理器的噪音抑制。
	{
		...
		int32_t m_NoiseSupes; //存放在噪音抑制时，噪音最大衰减分贝值，分贝值越小衰减越大，取值区间为[-2147483648,0]。
	...
	struct //存放Speex预处理器。
	{
		...
		int32_t m_VadProbStart; //存放在语音活动检测时，从无语音活动到有语音活动判断百分比概率，概率越大越难判断为有语音活，取值区间为[0,100]。
		int32_t m_VadProbCntu; //存放在语音活动检测时，从有语音活动到无语音活动判断百分比概率，概率越大越容易判断为无语音活动，取值区间为[0,100]。
		int32_t m_IsUseAgc; //存放是否使用自动增益控制，为非0表示要使用，为0表示不使用。
		int32_t m_AgcLevel; //存放在自动增益控制时，增益目标等级，目标等级越大增益越大，取值区间为[1,2147483647]。
		int32_t m_AgcIncrement; //存放在自动增益控制时，每秒最大增益分贝值，分贝值越大增益越大，取值区间为[0,2147483647]。
		int32_t m_AgcDecrement; //存放在自动增益控制时，每秒最大减益分贝值，分贝值越小减益越大，取值区间为[-2147483648,0]。
		int32_t m_AgcMaxGain; //存放在自动增益控制时，最大增益分贝值，分贝值越大增益越大，取值区间为[0,2147483647]。
	} m_SpeexPrpocs;
	...
	struct //存放Speex编码器。
	{
		...
		int32_t m_PlcExptLossRate; //存放在数据包丢失隐藏时，数据包预计丢失概率，预计丢失概率越高抗网络抖动越强、压缩率越低，取值区间为[0,100]。
	} m_SpeexEncd;

	struct //存放波形器。
	{
		int32_t m_IsDrawAdoWavfmToWnd; //存放是否绘制音频波形到窗口，为非0表示要绘制，为0表示不绘制。
		AdoWavfm * m_SrcWavfmPt; //存放原始波形器指针。
		AdoWavfm * m_RsltWavfmPt; //存放结果波形器指针。
		HWND m_RsltWavfmWndHdl; //存放结果波形窗口句柄。
		HWND m_SrcWavfmWndHdl; //存放原始波形窗口句柄。
	} m_Wavfm;
	
	struct //存放Wave文件。
	{
		int32_t m_IsSave; //存放是否保存，非0表示要保存，0表示不保存。
		WaveFileWriter * m_SrcWaveFileWriterPt; //存放原始Wave文件写入器指针。
		WaveFileWriter * m_RsltWaveFileWriterPt; //存放结果Wave文件写入器指针。
		Vstr * m_SrcFullPathVstrPt; //存放原始完整路径动态字符串指针。
		Vstr * m_RsltFullPathVstrPt; //存放结果完整路径动态字符串指针。
		size_t m_WrBufSzByt; //存放写入缓冲区大小，单位为字节。
	} m_WaveFile;

	struct //存放设备。
	{
		UINT m_ID; //存放标识符，取值范围为从1到音频输入设备总数，为0表示使用默认设备。
		IMediaObject * m_SystemAecNsAgcMediaObjPt; //存放系统自带声学回音消除器、噪音抑制器和自动增益控制器对象指针。
		DMO_OUTPUT_DATA_BUFFER * m_SystemAecNsAgcOtptDataBufPt; //存放系统自带声学回音消除器、噪音抑制器和自动增益控制器输出数据缓冲区对象指针。
		IMMDeviceEnumerator * m_EnumPt; //存放枚举器指针。
		IMMDeviceCollection * m_ClctPt; //存放收集器指针。
		IMMDevice * m_Pt; //存放指针。
		IAudioClient * m_ClntPt; //存放客户端指针。
		WAVEFORMATEX * m_WaveFmtExPt; //存放格式指针。
		UINT32 m_BufSzUnit; //存放缓冲区大小，单位为采样单元。
		IAudioCaptureClient * m_CptrClntPt; //存放捕获客户端指针。
		IGlobalInterfaceTable * m_GlblIntfcTablePt; //存放全局接口表指针。
		DWORD m_SystemAecNsAgcMediaObjCookie; //存放系统自带声学回音消除器、噪音抑制器和自动增益控制器对象Cookie。
		DWORD m_CptrClntCookie; //存放捕获客户端Cookie。
		BufQueueCls m_PcmBufFrmBufQueue; //存放Pcm格式缓冲区帧缓冲区队列。
		int16_t * m_PcmBufFrmPt; //存放Pcm格式缓冲区帧指针。
		size_t m_PcmBufFrmLenUnit; //存放Pcm格式缓冲区帧长度，单位为采样单元。
		size_t m_PcmBufFrmLenByt; //存放Pcm格式缓冲区帧长度，单位为字节。
		SpeexResamplerState * m_PcmSrcFrmSpeexResamplerPt; //存放Pcm格式原始帧Speex重采样器指针。
	...
	struct //存放线程。
	{
		...
		int16_t * m_PcmSrcFrmPt; //存放Pcm格式原始帧指针。
