...
typedef struct //音频输入。
{
	...
	struct //存放WebRtc浮点版声学回音消除器。
	{
		...
		int32_t m_Delay; //存放回音的延迟，单位为毫秒，取值区间为[-2147483648,2147483647]，为0表示自适应设置。
		...
		int32_t m_IsUseAdaptAdjDelay; //存放是否使用自适应调节回音的延迟，为非0表示要使用，为0表示不使用。
		int32_t m_IsSaveMemFile; //存放是否保存内存块到文件，为非0表示要保存，为0表示不保存。
		Vstr * m_MemFileFullPathVstrPt; //存放内存块文件完整路径动态字符串的指针。
	} m_WebRtcAec;
	...
	struct //存放SpeexWebRtc三重声学回音消除器。
	{
		...
		int32_t m_SpeexAecFilterLenMsec; //存放Speex声学回音消除器滤波器的长度，单位为毫秒。
	...
	struct //存放波形器。
	{
		...
		HWND m_RsltWavfmWndHdl; //存放结果波形窗口的句柄。
	...
	struct //存放Wave文件。
	{
		...
		WaveFileWriter * m_RsltWaveFileWriterPt; //存放结果Wave文件写入器的指针。
		Vstr * m_SrcWaveFileFullPathVstrPt; //存放原始Wave文件完整路径动态字符串的指针。
		Vstr * m_RsltWaveFileFullPathVstrPt; //存放结果Wave文件完整路径动态字符串的指针。
