...
typedef struct VdoOtptFrmElm //视频输出帧链表元素。
{
	uint32_t m_VdoOtptStrmIdx; //存放视频输出流索引。
	uint8_t * m_EncdVdoOtptSrcFrmPt; //存放已编码格式视频输出原始帧的指针，大小为 FrmMaxWidth * FrmMaxHeight * 3 / 2 字节。
	size_t m_EncdVdoOtptSrcFrmLenByt; //存放已编码格式视频输出原始帧的长度，单位为字节。
	uint8_t * m_YU12VdoOtptSrcFrmPt; //存放YU12格式视频输入结果帧的指针，大小为 FrmMaxWidth * FrmMaxHeight * 3 / 2 字节。
...
typedef struct VdoOtptStrm //视频输出流。
{
	...
	uint8_t * m_BgraVdoOtptRlstFrmPt; //存放BGRA8888格式视频输出结果帧的指针，大小为 FrmMaxWidth * FrmMaxHeight * 4 字节。
