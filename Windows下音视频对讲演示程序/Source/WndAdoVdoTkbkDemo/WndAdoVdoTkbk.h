...
//WndAdoVdoTkbk项目的DLL动态库文件导入导出符号宏。
	...
	#define __WNDADOVDOTKBK_DLLAPI__
			...
			#define __WNDADOVDOTKBK_DLLAPI__
		#endif
	#elif( ( defined __CYGWIN_GCC__ ) || ( defined __LINUX_GCC__ ) || ( defined __ANDROID_GCC__ ) || ( defined __KEIL_ARMC__ ) ) //如果正在使用Cygwin GCC/G++、Linux GCC/G++、Android GCC/G++、KEIL ARMCLANG/ARMCC编译器。
		#if( defined __COMLIB__ ) //如果正在编译LIB静态库文件。
			#define __WNDADOVDOTKBK_DLLAPI__
		#elif( defined __COMDLL__ ) //如果正在编译DLL动态库文件。
