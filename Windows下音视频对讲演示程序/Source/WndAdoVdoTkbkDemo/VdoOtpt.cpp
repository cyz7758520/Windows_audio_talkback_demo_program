...
{
	VdoOtpt * VdoOtptPt;
	VdoOtpt::VdoOtptStrm * VdoOtptStrmPt;
}VdoOtptThrdParm;
...
int VdoOtptVdoOtptStrmThrdInit( VdoOtpt * VdoOtptPt, VdoOtpt::VdoOtptStrm * VdoOtptStrmPt )
{
	...
	//初始化视频输出线程的临时变量。
	{
		...
		VdoOtptStrmPt->m_VdoOtptFrmPt = NULL; //初始化视频输出帧的指针。
...
void VdoOtptVdoOtptStrmThrdDstoy( VdoOtpt * VdoOtptPt, VdoOtpt::VdoOtptStrm * VdoOtptStrmPt )
{
	...
	if( VdoOtptStrmPt->m_IsInitVdoOtptThrdTmpVar != 0 )
	{
		...
		VdoOtptStrmPt->m_VdoOtptFrmPt = NULL; //销毁视频输出帧的指针。
...
DWORD WINAPI VdoOtptThrdRun( VdoOtptThrdParm * VdoOtptThrdParmPt )
{
	...
	VdoOtpt::VdoOtptStrm * p_VdoOtptStrmPt = VdoOtptThrdParmPt->VdoOtptStrmPt;
    ...
    {
		{
			if( p_VdoOtptStrmPt->m_VdoOtptFrmPt == NULL ) //如果没获取一个视频输出空闲帧。
			{
				//获取一个视频输出空闲帧。
				...
				if( p_VdoOtptStrmPt->m_FrmLnkLstElmTotal > 0 ) //如果视频输出空闲帧链表中有帧。
				{
					//从视频输出空闲帧链表中取出第一个帧。
					{
						...
						p_VdoOtptPt->m_VdoOtptIdleFrmLnkLst.GetHead( NULL, &p_VdoOtptStrmPt->m_VdoOtptFrmPt, NULL, 0, NULL );
				...
				else //如果视频输出空闲帧链表中没有帧。
				{
					...
					if( p_VdoOtptStrmPt->m_FrmLnkLstElmTotal <= 20 )
					{
						//创建一个视频输出空闲帧。
						{
							int p_CreateVdoOtptIdleFrmRslt = -1; //存放本处理段执行结果，为0表示成功，为非0表示失败。

							p_VdoOtptStrmPt->m_VdoOtptFrmPt = ( VdoOtpt::VdoOtptFrm * )calloc( 1, sizeof( VdoOtpt::VdoOtptFrm ) );
							if( p_VdoOtptStrmPt->m_VdoOtptFrmPt == NULL )
							{
								goto OutCreateVdoOtptIdleFrm;
							}
							p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt = ( uint8_t * )malloc( FrmMaxWidth * FrmMaxHeight * 3 / 2 );
							if( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt == NULL )
							{
								goto OutCreateVdoOtptIdleFrm;
							}
							if( p_VdoOtptStrmPt->m_UseWhatDecd != 0 )
							{
								p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt = ( uint8_t * )malloc( FrmMaxWidth * FrmMaxHeight * 3 / 2 );
								if( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt == NULL )
								{
									goto OutCreateVdoOtptIdleFrm;
								}
							}
							...
							if( p_CreateVdoOtptIdleFrmRslt != 0 ) //如果本处理段执行失败。
							{
								if( p_VdoOtptStrmPt->m_VdoOtptFrmPt != NULL )
								{
									if( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt != NULL ) free( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt );
									if( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt != NULL ) free( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt );
									free( p_VdoOtptStrmPt->m_VdoOtptFrmPt );
									p_VdoOtptStrmPt->m_VdoOtptFrmPt = NULL;
								}
							}
						}
					...
					{
						if( p_VdoOtptPt->m_MediaPocsThrdPt->m_IsPrintLog != 0 ) LOGFE( Cu8vstr( "视频输出线程：视频输出流索引 %uz32d：视频输出帧链表中帧总数为%uzd已经超过上限20，不再创建视频输出空闲帧。" ), p_VdoOtptStrmPt->m_VdoOtptStrmIdx, p_VdoOtptStrmPt->m_FrmLnkLstElmTotal );
						goto OutPocs;
					}
				}
			}
			...
			switch( p_VdoOtptStrmPt->m_UseWhatDecd ) //使用什么解码器。
			{
				...
				{
					//调用用户定义的写入视频输出帧函数。
					p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth = 0;
					p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight = 0;
					p_VdoOtptPt->m_MediaPocsThrdPt->m_UserWriteVdoOtptFrmFuncPt( p_VdoOtptPt->m_MediaPocsThrdPt, p_VdoOtptStrmPt->m_VdoOtptStrmIdx,
																				 p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt, &p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth, &p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight,
																				 NULL, 0, NULL );

					if( ( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth > 0 ) && ( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight > 0 ) ) //如果本次写入了视频输出帧。
					{
						if( p_VdoOtptPt->m_MediaPocsThrdPt->m_IsPrintLog != 0 ) LOGFI( Cu8vstr( "视频输出线程：视频输出流索引 %uz32d：使用YU12原始数据成功。YU12格式视频输出原始帧宽度：%z32d，高度：%z32d。" ), p_VdoOtptStrmPt->m_VdoOtptStrmIdx, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight );
					}
					else //如果本次没写入视频输出帧。
					{
						goto OutPocs;
				...
				{
					//调用用户定义的写入视频输出帧函数。
					p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmLenByt = 0;
					p_VdoOtptPt->m_MediaPocsThrdPt->m_UserWriteVdoOtptFrmFuncPt( p_VdoOtptPt->m_MediaPocsThrdPt, p_VdoOtptStrmPt->m_VdoOtptStrmIdx,
																				 NULL, NULL, NULL,
																				 p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt, FrmMaxWidth * FrmMaxHeight * 3 / 2, &p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmLenByt );

					if( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmLenByt > 0 ) //如果本次写入了视频输出帧。
					{
						//使用OpenH264解码器。
						if( OpenH264DecdPocs( p_VdoOtptStrmPt->m_OpenH264DecdPt,
											  p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmLenByt,
											  p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt, FrmMaxWidth * FrmMaxHeight * 3 / 2, &p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth, &p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight,
											  NULL ) == 0 )
						{
							if( p_VdoOtptPt->m_MediaPocsThrdPt->m_IsPrintLog != 0 ) LOGFI( Cu8vstr( "视频输出线程：视频输出流索引 %uz32d：使用OpenH264解码器成功。YU12格式视频输出原始帧宽度：%z32d，高度：%z32d。" ), p_VdoOtptStrmPt->m_VdoOtptStrmIdx, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight );
							if( ( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth == 0 ) || ( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight == 0 ) ) goto OutPocs; //如果未解码出YU12格式帧，就把本次视频输出帧丢弃。
						}
						else
						{
			...
			{
				int p_TmpLen = p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth * p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight;
				memset( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt, 0, p_TmpLen );
				memset( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt + p_TmpLen, 128, p_TmpLen / 2 );
			}

			//将本次YU12格式视频输出帧转为BGRA格式视频输出帧。
			if( LibYUVPictrFmtCnvrt( p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt, PICTR_FMT_BT601F8_YU12_I420, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight,
			...
			{
				//绘制视频输出帧到视频输出显示窗口。
				if( LibYUVPictrDrawToWnd( p_VdoOtptStrmPt->m_BgraVdoOtptRlstFrmPt, PICTR_FMT_SRGBF8_BGRA8888, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmWidth, p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmHeight,
										  0,
			...
			p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_VdoOtptStrmIdx = p_VdoOtptStrmPt->m_VdoOtptStrmIdx; //设置视频输出流索引。
			p_VdoOtptStrmPt->m_VdoOtptFrmPt->m_TimeStampMsec = p_VdoOtptStrmPt->m_LastTickMsec; //设置时间戳。

			//追加本次视频输出帧到视频输出帧链表。
			{
				p_VdoOtptPt->m_VdoOtptFrmLnkLst.PutTail( &p_VdoOtptStrmPt->m_VdoOtptFrmPt, 1, NULL );
				p_VdoOtptStrmPt->m_VdoOtptFrmPt = NULL;
			}
	...
	Out:
	if( p_VdoOtptStrmPt->m_VdoOtptFrmPt != NULL ) //如果获取的视频输出空闲帧没有追加到视频输出帧链表。
	{
		p_VdoOtptPt->m_VdoOtptIdleFrmLnkLst.PutTail( &p_VdoOtptStrmPt->m_VdoOtptFrmPt, 1, NULL );
		p_VdoOtptStrmPt->m_VdoOtptFrmPt = NULL;
	}
...
int VdoOtptInit( VdoOtpt * VdoOtptPt )
{
	...
	//初始化视频输出帧链表。
	if( VdoOtptPt->m_VdoOtptFrmLnkLst.Init( sizeof( VdoOtpt::VdoOtptFrm * ), BufAutoAdjMethFreeNumber, 1, 0, 1, SIZE_MAX, VdoOtptPt->m_MediaPocsThrdPt->m_ErrInfoVstrPt ) == 0 )
	{
	...
	//初始化视频输出空闲帧链表。
	if( VdoOtptPt->m_VdoOtptIdleFrmLnkLst.Init( sizeof( VdoOtpt::VdoOtptFrm * ), BufAutoAdjMethFreeNumber, 1, 0, 1, SIZE_MAX, VdoOtptPt->m_MediaPocsThrdPt->m_ErrInfoVstrPt ) == 0 )
	{
...
{
	VdoOtpt::VdoOtptStrm * p_VdoOtptStrmPt; //存放视频输出流的指针。
	VdoOtpt::VdoOtptFrm * p_VdoOtptFrmPt; //存放视频输出帧的指针。
	...
	{
		while( VdoOtptPt->m_VdoOtptIdleFrmLnkLst.GetHead( NULL, &p_VdoOtptFrmPt, NULL, 0, NULL ) == 0 )
		{
			if( p_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt )
			{
				free( p_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt );
				p_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt = NULL;
			}
			if( p_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt != NULL )
			{
				free( p_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt );
				p_VdoOtptFrmPt->m_EncdVdoOtptSrcFrmPt = NULL;
			}
			free( p_VdoOtptFrmPt );
			p_VdoOtptFrmPt = NULL;
	...
	{
		while( VdoOtptPt->m_VdoOtptFrmLnkLst.GetHead( NULL, &p_VdoOtptFrmPt, NULL, 0, NULL ) == 0 )
		{
			if( p_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt )
			{
				free( p_VdoOtptFrmPt->m_YU12VdoOtptSrcFrmPt );
