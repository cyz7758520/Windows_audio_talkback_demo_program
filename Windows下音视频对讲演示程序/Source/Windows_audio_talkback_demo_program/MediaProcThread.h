...
typedef struct MediaProcThread
{
	...
	typedef struct AudioInput //音频输入结构体。
	{
		...
		int32_t m_SpeexWebRtcAecIsUseSameRoomAec; //存放SpeexWebRtc三重声学回音消除器是否使用同一房间声学回音消除，可能对近端音频有损伤，为非0表示要使用，为0表示不使用。
		...
		ConstLenLnkLst * m_AudioInputFrameLnkLstPt; //存放音频输入帧链表的内存指针。
		ConstLenLnkLst * m_AudioInputIdleFrameLnkLstPt; //存放音频输入空闲帧链表的内存指针。
	...
	typedef struct AudioOutput //音频输出结构体。
	{
		...
		ConstLenLnkLst * m_AudioOutputFrameLnkLstPt; //存放音频输出帧链表的内存指针。
		ConstLenLnkLst * m_AudioOutputIdleFrameLnkLstPt; //存放音频输出空闲帧链表的内存指针。
	...
	typedef struct VideoInput
	{
		...
		ConstLenLnkLst * m_VideoInputFrameLnkLstPt; //存放视频输入帧链表的内存指针。
		ConstLenLnkLst * m_VideoInputIdleFrameLnkLstPt; //存放视频输入空闲帧链表的内存指针。
