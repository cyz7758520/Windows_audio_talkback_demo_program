...
//MediaFile项目的DLL动态库文件导入导出符号宏。
			...
			#define __MEDIAFILE_DLLAPI__
		#elif( defined __COMDLL__ ) //如果正在编译DLL动态库文件。
			#define __MEDIAFILE_DLLAPI__ __declspec( dllexport )
			...
			#define __MEDIAFILE_DLLAPI__
		#elif( defined __COMDLL__ ) //如果正在编译DLL动态库文件。
			#define __MEDIAFILE_DLLAPI__ __attribute__( ( visibility( "default" ) ) )
		#elif( defined __COMEXE__ ) //如果正在编译EXE可执行文件。
			#define __MEDIAFILE_DLLAPI__
		#endif
	#else //如果正在使用未知编译器。
		#define __MEDIAFILE_DLLAPI__
			...
			#define __MEDIAFILE_DLLAPI__ __declspec( dllimport )
		#endif
	#elif( ( defined __LINUX_GCC__ ) || ( defined __CYGWIN_GCC__ ) || ( defined __ANDROID_GCC__ ) || ( defined __KEIL_ARMC__ ) ) //如果正在使用Cygwin GCC/G++、Linux GCC/G++、Android GCC/G++、KEIL ARMCLANG/ARMCC编译器。
		#define __MEDIAFILE_DLLAPI__
	#else //如果正在使用未知编译器。
